...
static const struct inode_operations vfat_file_inode_operations = {
	.setattr	= vfat_android_setattr,
	.getattr	= vfat_android_getattr,
};
...
static int set_mode(struct file *filp, unsigned const char *filename, mode_t mode, uid_t uid, gid_t gid)
{
	...
	{
		//printk("set_mode:entry filename = %s; filename = %s\n", entry.filename, filename);
		...
		{	
			...
			entry.uid = uid;
			entry.gid = gid;	
			filp->f_op->llseek(filp, -len, SEEK_CUR);
			filp->f_op->write(filp, (void *)&entry, len, &filp->f_pos);
...
static int new_permission_entry(struct file *filp, unsigned const char *filename, mode_t mode, uid_t uid, gid_t gid)
{
	...
	entry.uid = uid;
	entry.gid = gid;
	if(get_file_entry(filp, filename, &mode, &uid, &gid) > 0)
		filp->f_op->llseek(filp, -(sizeof (struct permission_entry)), SEEK_CUR);
	else
		filp->f_op->llseek(filp, 0, SEEK_END);
	return filp->f_op->write(filp, (void *)&entry, sizeof entry, &filp->f_pos);
	
}


void get_filepath(struct dentry *dentry, char *filepath)
{
	...
	strcat(tmp, filepath);
	strcpy(filepath, tmp);
	strcat(filepath, "/.permissions");
	printk("GET FILEPATH::: %s\n", filepath);
	return ;
}
...
//		filp = my_do_filp_open(nd, 00, O_RDONLY); 
	...
	if(!(inode->i_mode & S_IFDIR))
		inode->i_op = &vfat_file_inode_operations;
	brelse(sinfo.bh);
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		goto error;
	}
	printk("mode is octal= %o\n", inode->i_mode);
	printk("uid is = %X\n", inode->i_uid);
	printk("gid is = %X\n", inode->i_gid);
	inode->i_mode = inode->i_mode & ~(0777);
	inode->i_mode = inode->i_mode | mode;
	if(uid)
		inode->i_uid = uid;
	if(gid)
		inode->i_gid = gid;
	printk("changed mode is octal= %o\n", inode->i_mode);
	printk("changed uid is = %d\n", inode->i_uid);
	printk("changed gid is = %d\n", inode->i_gid);
		       ...
		       struct nameidata *nd)
{
	...
	if(strstr(dentry->d_name.name, ".permissions") == NULL)
	{
		...
		get_filepath(dentry, filepath);	
		mutex_unlock(&(dentry->d_parent->d_inode->i_mutex));		
		filp = filp_open(filepath, O_RDWR, 00);
		mutex_lock(&(dentry->d_parent->d_inode->i_mutex));		
		if(!(IS_ERR(filp) || filp == NULL ||filp->f_op->read == NULL ))
		{
			new_permission_entry(filp, dentry->d_name.name, inode->i_mode, inode->i_uid, inode->i_gid);
			fput(filp);
		}
		else
			printk("FAIL FAIL FAIL\n");
...
static int vfat_android_setattr(struct dentry *dentry, struct iattr *attr)
{
	int error, setmode = 0, setuid = 0, setgid = 0;
	struct file *filp;
	struct inode * inode = dentry->d_inode;
...
//	error = fat_setattr(dentry, attr);
	...
	if(setmode)
	{
		attr->ia_valid = attr->ia_valid | ATTR_MODE;
		attr->ia_mode = attr->ia_mode | mode;
	}
	if(setuid)
	{
		attr->ia_valid = attr->ia_valid | ATTR_UID;
		attr->ia_mode = attr->ia_uid | uid;
	}
	if(setgid)
	{
...
{
	printk("Registering special android fs\n");
